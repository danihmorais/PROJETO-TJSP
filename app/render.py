...
def render_markdown(entries: list[CompilationEntry], generated_at: datetime) -> str:
    ...
    for entry in entries:
        ...
        lines += [f"### {entry.source.title}", ""]
        lines += [f"**Fonte oficial:** {entry.source.url}", ""]
        if entry.error:
...
h3 {{ font-family: system-ui, sans-serif; margin-bottom: 4px; }}
h4 {{ font-family: system-ui, sans-serif; margin-bottom: 8px; }}
.subtitle, .source, .consulted {{ color: #5b6470; font-size: .92rem; }}
.article {{ margin: 22px 0; }}
.article p {{ margin: 5px 0; }}
.law {{ page-break-inside: auto; }}
