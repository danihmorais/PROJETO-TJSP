...
def _caput_preview(device: Device, limit: int = 180) -> str:
        ...
        if entry.error:
            ...
            if entry.devices:
                ...
                for device in entry.devices:
                    ...
                    quick_items.append(
                        f"<a href='#{article_id}' class='quick-item'><strong>Art. {escape(device.number)}</strong>"
                        f"<span>{escape(_caput_preview(device))}</span></a>"
                    )
                body.append(
                    "<details class='quick-map' open><summary>Mapa rápido dos artigos</summary>"
