...
PROJECTS = {
    "DANIHMORAIS-GITHUB-PAGES": {
        ...
        "script": "/home/daniel/python/deploy-site.sh",
    },
    "PROJETO-TJSP": {
...
def save_status(project, sha, state, message=""):
    ...
    tmp.write_text(json.dumps({"project":project,"sha":sha,"state":state,"message":message}, ensure_ascii=False), encoding="utf-8")
    tmp.replace(path)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            subprocess.Popen([PROJECTS[project]["script"], sha], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, start_new_session=True)
